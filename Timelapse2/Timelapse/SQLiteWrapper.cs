...
class SQLiteWrapper
{
        ...
        for (int i = 0; i < datatable.Rows.Count; i++)
        {
            ...
            for (int j = 0; j < datatable.Columns.Count; j++)
            {
                ...
                else
                {
                    ...
                    values += String.Format(" {0}" + COMMA, quote(newvalue));         //  "'value1', 'value2', ... 'valueN'"
                }
            }
    ...
    public void InsertMultiplesBeginEnd(String table_name, List<Dictionary<String, String>> insertion_statements, out bool result, out string command_executed)
    {
        ...
        foreach (Dictionary<String, String> data in insertion_statements)
        {
            ...
            if (columns.Length > 0) columns = columns.Substring(0, columns.Length - COMMA.Length);     // Remove last comma in the sequence to produce (col1, col2, ... coln)  
            if (values.Length > 0) values = values.Substring(0, values.Length - COMMA.Length);        // Remove last comma in the sequence 
    ...
    public void UpdateColumnTrimWhiteSpace(String table_name, List<string> column_names, out bool result, out string command_executed)
    {
        ...
        foreach (string column_name in column_names)
        {
            ...
            ExecuteNonQuery(command_executed, out result);  //TODO MAKE THIS ALL IN ONE OPERATION
        }
    }
    ...
    public void UpdateWhereBeginEnd(String table_name, List<ColumnTupleListWhere> update_query_list, out bool result, out string command_executed)
    {
        ...
        {
            query = UpdateCreateSingleUpdateQuery(table_name, ctlw.Listpair, ctlw.Where);
            if (query.Equals("")) continue; // skip non-queries
            queries.Add(query);
            command_executed += query; // The string of queries
        }
    ...
    private string UpdateCreateSingleUpdateQuery(String table_name, ColumnTupleList columnname_value_list, String where)
    {
        // UPDATE table_name SET 
        // colname1 = value1, 
        // colname2 = value2,
        ...
        foreach (ColumnTuple val in columnname_value_list)                          // column_name = 'value',
        {
            // we have to cater to different formats for integers, NULLS and strings...
            ...
            {
                cells_to_update += String.Format(" {0} = {1}{2}", val.ColumnName.ToString(), quote(val.ColumnValue.ToString()), COMMA);
            }
            else
            {
                // This shouldn't happen, but just in case...
