...
namespace Carnassial.UnitTests
{
    ...
    public class UserInterfaceTests : CarnassialTest
    {
            ...
            lock (UserInterfaceTests.App)
            {
                using (CarnassialWindow carnassial = new CarnassialWindow())
                {
                    ...
                    using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        Marker newMarker = new Marker(TestConstant.DefaultDatabaseColumn.Counter0, default) { LabelShownPreviously = false, ShowLabel = true };
        ...
        {
            // open, do nothing, close
            using (EditorWindow editor = new EditorWindow())
            {
                editor.Show();
                this.WaitForRenderingComplete();
                editor.Close();
