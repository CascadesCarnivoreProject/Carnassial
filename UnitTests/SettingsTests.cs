...
namespace Carnassial.UnitTests
{
    ...
    public class SettingsTests : CarnassialTest
    {
        ...
        {
            // all tests sensitive to user settings changes lock on CarnassialTest.SettingsLock (here, DatabaseTests, potentially UITestMethodAttribute)
            // Locking on CarnassialSettings.Default causes WPF UX hangs, presumably due to another lock internal to WPF deadlocking.
            CarnassialTest.AppLock.Wait(this.TestContext!.CancellationToken);
            try
            {
